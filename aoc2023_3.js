...
const isAdjacentToSymbol = (digitCoordinates, input) => {
    for (let coordinate of digitCoordinates) {
        for (let dir of directions) {
            const newX = coordinate.x + dir.dx;
            const newY = coordinate.y + dir.dy;

            if (newX >= 0 && newX < input.length && newY >= 0 && newY < input[newX].length) {
                if (specialCharRegex.test(input[newX][newY])) {
                    return true;
                }
            }
        }
...
const getDigits = (x, y, input) => {
    let coordinates = [];
    while (y < input[x].length && !isNaN(input[x][y]) && input[x][y] !== ' ') {
        coordinates.push(new Coordinate(x, y, input[x][y]));
        y++;
    }
    return coordinates;
}
...
const partOne = (input) => {
    ...
    for (let x = 0; x < input.length; x++) {
        for (let y = 0; y < input[x].length; y++) {
            if (!isNaN(input[x][y]) && input[x][y] !== ' ') {
                let digitCoordinates = getDigits(x, y, input);

                if (digitCoordinates.length > 0 && isAdjacentToSymbol(digitCoordinates, input)) {
                    let number = digitCoordinates.map(coord => coord.value).join('');
                    total += +number;
                    y += digitCoordinates.length - 1;
                }
            }
        }
    }
...
const getDigitsPartTwo = (x, y) => {
    ...
    while (newY > 0 && !isNaN(input[x][newY - 1]) && input[x][newY - 1] !== ' ') {
        newY--;
    }

    const digits = [];
    while (newY < input[x].length && !isNaN(input[x][newY]) && input[x][newY] !== ' ') {
        digits.push(input[x][newY]);
        newY++;
    }
        ...
        for (let y = 0; y < input[x].length; y++) {
            if (input[x][y] == '*') {
                ...
                for (let direction of directions) {
                    ...
                    if (newX >= 0 && newY >= 0 && newX < input.length && newY < input[newX].length) {
                        const check = input[newX][newY];
                        if (!isNaN(+check)) {
                            let digits = getDigitsPartTwo(newX, newY);
                            if (digits != '') {
                                digits = { digits: digits, gearCoordinate: { x: x, y: y } };
                                adjacents.push(digits);
                            }
                        }
                    }
                }

                let uniques = Array.from(new Set(adjacents.map(JSON.stringify))).map(JSON.parse);
                if (uniques.length == 2) {
                    const power = uniques[0].digits * uniques[1].digits;
                    total += power;
                }
            }
        }
