...
function partOne(input) {
    ...
    for (let i = 0; i < input.length; i++) {
        let matches = [...input[i].matchAll(/\d/g)].map(x => {
            return { index: x.index, value: x[0] };
        })

        matches.sort((a, b) => a.index - b.index);

        let numbersMatched = [];
        for (let j = 0; j < Object.keys(numberMap).length; j++) {
            var key = Object.keys(numberMap)[j];
            var matchTest = input[i].match(key);
            numbersMatched.push(matchTest);
        }

        numbersMatched = numbersMatched.filter(x => x != null)

        let numstrmatchmap = [];
        for (let idx = 0; idx < numbersMatched.length; idx++) {
            const elem = numbersMatched[idx];
            const ob = {
                index: elem["index"],
                value: elem[0]
            }
            numstrmatchmap.push(ob);
        }

        matches = [...matches, ...numstrmatchmap];
        matches.sort((a, b) => a.index - b.index);

        let first = 0;
        let second = 0;

        const firstelem = matches[0].value;
        const lastelem = matches[matches.length - 1].value;

        const isStringRepresentationFirst = numberMap[firstelem] != undefined;
        const isStringRepresentationSecond = numberMap[lastelem] != undefined;

        first = isStringRepresentationFirst ? numberMap[firstelem] : parseInt(firstelem, 10);
        second = isStringRepresentationSecond ? numberMap[lastelem] : parseInt(lastelem, 10);

        const final = parseInt(`${parseInt(first, 10)}${parseInt(second, 10)}`, 10);
